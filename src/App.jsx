...
export default function App() {
 ...
 // Keepalive
 const [keepaliveStatus,setKeepaliveStatus]=useState(null); // null | "ok" | "error"

 // ── Supabase keepalive (pings every 6 days on app open to prevent 7-day inactivity pause) ──
 useEffect(()=>{
 async function ping(){
 const INTERVAL_MS=6*24*60*60*1000; // 6 days
 const lastPing=localStorage.getItem("rosette_last_ping");
 const now=Date.now();
 if(lastPing&&now-Number(lastPing)<INTERVAL_MS)return; // not due yet
 try{
 const{error}=await supabase.from("supplements").select("id").eq("user_id",UID).limit(1);
 if(error)throw error;
 localStorage.setItem("rosette_last_ping",String(now));
 setKeepaliveStatus("ok");
 }catch(e){
 ...
 *{box-sizing:border-box;margin:0;padding:0}
 ...
 {keepaliveStatus&&(
 <span title={keepaliveStatus==="ok"?"Database connection active":"Database ping failed — check connection"} style={{width:6,height:6,borderRadius:"50%",background:keepaliveStatus==="ok"?"#34d399":"#fb923c",display:"inline-block",marginBottom:2,flexShrink:0,boxShadow:keepaliveStatus==="ok"?"0 0 0 2px #34d39930":"0 0 0 2px #fb923c30"}}/>
 )}
 </div>
